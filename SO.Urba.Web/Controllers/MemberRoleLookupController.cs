...
namespace  SO.Urba.Controllers
{
    ...
    public class MemberRoleLookupController : Controller
    {
		...
		public ActionResult Index(SearchFilterVm input = null, Paging paging = null)
        {
            ...
            { 
                ...
                input = memberRoleLookupManager.search(input);
                return View(input);
            }
            return View(input);
        }
        ...
        {
            var results = memberRoleLookupManager.getAll(null);
            return PartialView(results);
        }

        [HttpPost]
        public ActionResult Edit(Guid id, MemberRoleLookupVo input)
        {

            if (this.ModelState.IsValid)
            {
                var res = memberRoleLookupManager.update(input, id);
        ...
        {
            var result = memberRoleLookupManager.get(id);
            return View(result); 
        }

        [HttpPost]
        public ActionResult Create(MemberRoleLookupVo input)
        {

            if (this.ModelState.IsValid)
            {

                var item = memberRoleLookupManager.insert(input);
                return RedirectToAction("Index");
            }
        ...
        {
            memberRoleLookupManager.delete(id);
            return RedirectToAction("Index");
        }
